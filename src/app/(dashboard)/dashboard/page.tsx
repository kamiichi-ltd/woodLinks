...
export default async function DashboardPage() {
    ...
    if (!user) {
        // Handle no user effectively (though middleware should catch this)
    ...
    return (
        <div className="space-y-12 relative min-h-screen pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 px-2 sm:px-4">
                ...
                {/* Left Column: Create & Analytics (Sticky or Stacked) */}
                    ...
                    {cards.length > 0 && (
                        <section className="space-y-4">
                            <div className="flex items-center gap-2 mb-2 px-2">
                                <div className="w-1.5 h-6 bg-[#2c3e50] rounded-full"></div>
                                <h3 className="text-xl font-serif font-bold text-[#2c3e50] tracking-tight">Analytics</h3>
                            </div>

                            {/* Summary Cards */}
                            <AnalyticsSummary totalViews={analytics.totalViews} totalSaves={analytics.totalSaves} />
